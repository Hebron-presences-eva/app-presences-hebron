...
app.post('/api/login', async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('membres')
      .select('id, nom, groupe_id, role') // ⬅️ Ajouter role
      .eq('identifiant', identifiant)
      .eq('mot_de_passe', mot_de_passe);
    ...
    if (data && data.length > 0) {
      console.log("✅ Connexion réussie pour:", identifiant, "Role:", data[0].role);
      return res.json(data[0]); // Inclut maintenant le role
    }
...
// Middleware pour vérifier les permissions
function verifierPermissions(req, res, next) {
  // Récupérer le membre depuis le localStorage (simulé via un header)
  const membreJson = req.headers['x-membre-data'];
  ...
  try {
    ...
    req.membre = membre; // Attacher le membre à la requête
    next();
  } catch (err) {
...
// Fonction pour vérifier l'accès au groupe
function verifierAccesGroupe(req, res, next) {
  const membre = req.membre;
  const groupeId = parseInt(req.query.groupe || req.query.groupe_id, 10);
  
  // Si admin, autoriser tout
  if (membre.role === 'admin') {
    console.log("✅ Admin détecté, accès autorisé à tous les groupes");
    return next();
  }
  
  // Si membre normal, vérifier qu'il accède à son propre groupe
  if (membre.groupe_id !== groupeId) {
    console.log(`❌ Accès refusé: membre groupe ${membre.groupe_id} essaie d'accéder au groupe ${groupeId}`);
    return res.status(403).json({ 
      error: "Accès refusé : vous ne pouvez accéder qu'à votre propre groupe" 
    });
  }
...
app.get('/api/groupes', verifierPermissions, async (req, res) => {
  try {
    const membre = req.membre; // Récupéré par le middleware
    ...
    // Transformer libelle → nom
    let groupesTransformed = data.map(groupe => ({
      id: groupe.id,
      nom: groupe.libelle
    }));
    ...
    // 🔒 FILTRAGE PAR RÔLE
    if (membre.role !== 'admin') {
      // Membre normal : ne voir que son groupe
      groupesTransformed = groupesTransformed.filter(g => g.id === membre.groupe_id);
      console.log(`🔐 Membre normal ${membre.nom} : accès limité au groupe ${membre.groupe_id}`);
    } else {
      console.log(`👑 Admin ${membre.nom} : accès à tous les ${groupesTransformed.length} groupes`);
    }
...
app.get('/api/membres', verifierPermissions, verifierAccesGroupe, async (req, res) => {
...
app.get('/api/presences/resume', verifierPermissions, verifierAccesGroupe, async (req, res) => {
...
app.get('/api/presences/annuel', verifierPermissions, verifierAccesGroupe, async (req, res) => {
...
app.get('/api/presences', verifierPermissions, verifierAccesGroupe, async (req, res) => {
...
app.get('/api/absences', verifierPermissions, verifierAccesGroupe, async (req, res) => {
  const { groupe, mois, annee, reunion_id } = req.query;
...
app.get('/api/retards', verifierPermissions, verifierAccesGroupe, async (req, res) => {
  const { groupe, mois, annee, reunion_id } = req.query;
...
app.post('/api/retards', verifierPermissions, verifierAccesGroupe, async (req, res) => {
...
app.delete('/api/retards/:id', verifierPermissions, verifierAccesGroupe, async (req, res) => {
  const id = parseInt(req.params.id, 10);
...
app.get('/api/reactivite', verifierPermissions, verifierAccesGroupe, async (req, res) => {
...
// Route résumé étendu
app.get('/api/resume-etendu', verifierPermissions, verifierAccesGroupe, async (req, res) => {
  const { groupe, mois, annee, reunion_id } = req.query;
